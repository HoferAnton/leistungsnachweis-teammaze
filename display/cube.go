...
func generateAndInitializeBuffers(cube *Cube) {
	...
	numVertexBufferBytes := len(vertices) * int(unsafe.Sizeof(mgl32.Vec3{}))
	gl.BufferData(gl.ARRAY_BUFFER, numVertexBufferBytes, gl.Ptr(vertices), gl.STATIC_READ)
	gl.BufferData(gl.ELEMENT_ARRAY_BUFFER, len(drawingIndices), gl.Ptr(drawingIndices), gl.STATIC_READ)
}
...
func (cube Cube) draw(view mgl32.Mat4, projection mgl32.Mat4, labCenter mgl32.Vec3, lightPosition mgl32.Vec3, timeSinceStart time.Duration) {
	...
	model := mgl32.HomogRotate3DY(float32(timeSinceStart.Seconds())).
		Mul4(mgl32.Translate3D(-labCenter.X(), -labCenter.Y(), -labCenter.Z()).Mul4(cube.Transform.AsMatrix()))
	...
	gl.DrawElements(gl.TRIANGLES, int32(len(drawingIndices)), gl.UNSIGNED_BYTE, gl.PtrOffset(0))
...
func (cube Cube) String() string {
	...
	return fmt.Sprintf("Cube at (%v) with %s vao.", cube.Transform.translation.Col(3), vaoValidness)
}
